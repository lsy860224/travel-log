...
> 일정 카테고리 **기본** 색. 종이·잉크와 어울리는 **빈티지 어스톤**으로 통일 — 형광·고채도 금지.
> 기본값 `index.html` 의 `DEFAULT_CATS`. 사용자가 앱에서 직접 추가·수정 가능(`travel-log:cats`) — 커스텀 색도 이 톤을 권장. 미지정/삭제 일정은 타입별 기본색(`--stamp`/`--sea`)으로 폴백.

| 카테고리 | hex | 비고 |
|---|---|---|
| 가족 | `#b84a32` | = `--stamp` (브랜드 레드 재사용) |
| 여행 | `#4d7d8e` | = `--sea` |
| 약속 | `#c0823a` | 오커 / 앰버 |
| 기념일 | `#a2566b` | 더스티 로즈 |
| 건강 | `#6e8b5a` | 세이지 그린 |
...
새 카테고리 추가 시 위 톤(채도 낮은 따뜻한 색)을 유지하고 이 표에 등록할 것.
